...
#define SUITE_NAME Blink1Device_BadInit_test
...
class SUITE_NAME : public ::testing::Test {
    protected:
        void SetUp() override {
            ...
            fake_blink1_lib::SET_BLINK1_SUCCESSFUL_INIT(false);
        }
...
TEST_F(SUITE_NAME, TestGood) {
    {
        Blink1Device device;
        EXPECT_FALSE(device.good()) << "Expected device.good() to return false";
    }
    checkDevicesFreed();
}
...
TEST_F(SUITE_NAME, TestOperatorBool) {
    {
        Blink1Device device;
        EXPECT_FALSE(bool(device)) << "Expected bool(device) to be false";
    }
    checkDevicesFreed();
}
...
TEST_F(SUITE_NAME, TestGetVersion) {
    {
        ...
        EXPECT_FALSE(deviceVersion) << "Expected device version not to be present";
    }
    checkDevicesFreed();
}
